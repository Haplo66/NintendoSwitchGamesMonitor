...
export function resolveRunMode(argv: string[]): ResolvedRunMode {
  ...
  const forceDigestEmail = args.includes('--force-email') || args.includes('test-email');
  return { dryRun, forceDigestEmail };
}

export async function runLocalMonitor(argv: string[] = process.argv): Promise<void> {
  const mode = resolveRunMode(argv);

  if (mode.dryRun && mode.forceDigestEmail) {
    throw new Error(
  ...
  const options: MonitorOptions = {
    dryRun: mode.dryRun,
    forceDigestEmail: mode.forceDigestEmail,
  };
