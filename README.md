...
11. **Monitoring Statistics** (optional) — games checked/reported/skipped, the collector, and execution time.
12. **Footer** — a muted "generated automatically" note.
...
3. **Filter** — only games worth reporting are kept. A game is included when it is free and matches a family profile (if `notifyFreeGames`), matches a wishlist item (if `notifyWishlistMatches`), or its deal score reaches `minimumDealScore`. Free games never need the score threshold.
...
Monitor summary:
  ...
  New notifications: 0
  Skipped cooldown: 3
  Email: sent (forceDigestEmail=true)
```
...
3. **Deal scorer** (`src/analyzer/deal-score.ts`) — scores a deal on: discount percentage (higher = better), free games (strong bonus), wishlist match (strong bonus), a **price-target-reached** bonus (on top of wishlist match), a **historical-low** bonus (applied at digest build time, once the price history is available — a deal at its lowest-ever price scores +15 and sorts higher in Best Deals), and family profile matches (bonus). Returns `DealScoreResult` (`score`, `reasons`). Internal scoring can exceed **100** so high-value deals keep their ranking edge; the **displayed** score (email **Deal Score:** badge and Markdown **Deal score:**) is capped at **100** via `displayScore()` so it stays intuitive.
...
- `notifyFreeGames` — when `true`, a free-to-play game is reported only if it matches at least one family profile (age rating within range, no excluded genre). When `false`, free games are never reported. Free games never need to reach `minimumDealScore`.
...
`dailyDigest`, `sendEmptyDigest` and `forceDigestEmail` are configured **only** in `data/settings.json` (no environment variables); a missing or partial `dailyDigest` block merges with the defaults above. `DRY_RUN` / `--force-email` are **NOT** in this table: they are one-time execution modes / per-run overrides (see below). `forceDigestEmail` (the persisted setting) is configured only in `data/settings.json` (default `false`).
...
| `data/settings.json` | Application settings: notification preferences (incl. `forceDigestEmail`) + user preferences (`platform`, `emailProvider`, `gameCollector`, `logLevel`, `emailTo`) | Application settings |
