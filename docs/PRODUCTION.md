...
The digest surfaces this context only when it is useful: **⭐ At its historical low** (with the previous low) when the current price is the best ever, or **Historical low: $X** when a cheaper historical price exists — otherwise no extra line is shown.

The same history also powers a one-glance **deal quality** rating on deal cards. `evaluateDealQuality` (in `src/history/deal-quality.ts`) maps the current price against the historical low and average to `excellent` (at its historical low), `great` (within ~10% of the low), `good` (below the historical average), or `weak` (at/above the average), each with a short reason. Games with no useful history get no badge. When both are present, the rating badge takes precedence over the quieter price-context line.
