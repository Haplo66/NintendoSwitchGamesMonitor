...
Features:
- **User preferences leave `.env`** — `platform`, `emailProvider`, `dryRun`, `forceDigestEmail`, and `logLevel` are now configured in `data/settings.json` instead of `.env`; `NINTENDO_PLATFORM`, `EMAIL_PROVIDER`, `DRY_RUN`, `FORCE_EMAIL`, and `LOG_LEVEL` were removed from `.env.example` (which now keeps only secrets and environment-specific values)
...
- **Runtime logging** — startup now prints the resolved values: `Dry run: enabled/disabled` and `Force digest email: enabled/disabled` (alongside Collector/Region/Platform/Email/score/cooldown), so the effective configuration is visible each run
...
- **Sender is always `SMTP_USER`** — `MAIL_FROM` removed entirely; `mailFrom()` derives the From address from the SMTP user (single source of truth). The optional `emailTo` preference (recipient) defaults to `SMTP_USER` when unset
- **Recipient migration completed (no `EMAIL_TO` anywhere)** — the last `EMAIL_TO` trace was removed: the preferences resolver no longer honors an `EMAIL_TO` env override, so the digest recipient comes **only** from `emailTo` in `data/settings.json` (falling back to `SMTP_USER`). `GmailProvider.fromEnv` requires just `SMTP_USER` + `SMTP_PASSWORD`, the workflow no longer mentions `EMAIL_TO`, and `validate-preferences` (17 checks) + `validate-production` assert the provider, workflow, and scheduled environment never depend on it
- **Runtime modes are one-time flags, not settings** — `DRY_RUN` / `FORCE_EMAIL` are gone from `.env` and `data/settings.json` (the `dryRun` / `forceDigestEmail` preference keys were removed); they are now passed per run as `npm run monitor -- --dry-run` / `-- --force-email` (parsed by `resolveRunFlags` in `monitor-run.ts` and `resolveRunMode` in `run-monitor.ts`, which rejects combining both) or as GitHub Actions manual-dispatch inputs that build `$EXTRA_FLAGS`
...
- **Validation** — `validate-preferences` expanded to 17 checks (defaults, settings load, partial merge, env override, normalization, invalid rejection, app-config integration, `.env.example` key set, Gmail host/port/secure defaults, required SMTP credentials, `mailFrom` derivation, `emailTo` fallback, no-`EMAIL_TO`-required, settings-only recipient, provider recipient wiring, runtime flag parsing); `validate-dry-run` / `validate-force-email` now assert the flags are one-time CLI modes; `validate-production` asserts the workflow exports no `DRY_RUN`/`FORCE_EMAIL`, never mentions `EMAIL_TO`, and that secrets are only `SMTP_USER`/`SMTP_PASSWORD`; `npm test` aggregates everything (build + 16 suites)
...
Features:
- **Persisted `forceDigestEmail` notification setting** — new `forceDigestEmail` key in `data/settings.json` (boolean, default `false`). When `true`, the daily digest email is always sent even when every notification is in cooldown or there are no new notifications
- **Config plumbing** — `NotificationSettings` gains `forceDigestEmail`, `settings-loader` adds it to defaults + boolean validation + resolution (no environment override, so `.env` stays secrets-only); `monitor-run` now reads `config.notification.forceDigestEmail` and falls back to it when the one-time `--force-email` flag is not passed
...
- **Validation** — `validate-settings` covers the `forceDigestEmail` default, full-file preservation, partial-merge fallback, and non-boolean rejection; `validate-force-email` keeps the CLI + decision-logic coverage (reason now `forceDigestEmail=true`); `npm test` aggregates everything
